...
// CORS restringido a los dominios del portal
const ALLOWED_ORIGINS = ['https://portal.alsalgeciras.com', 'https://portal.customsway.eu'];
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({ ok: true, ...emailResult }), {
      headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': corsOrigin(req) }
    });
  } catch (err) {
